...
class ProductInCart(models.Model):
    product = models.ForeignKey(
        to="commerce.Product", on_delete=models.CASCADE, verbose_name="Товар"
    )
...
class OrderItem(models.Model):
    product = models.ForeignKey(
        to="commerce.Product", on_delete=models.PROTECT, verbose_name="Товар"
    )
    size = models.CharField(max_length=10, blank=True, null=True, verbose_name="Размер")
    number = models.PositiveIntegerField(verbose_name="Количество")
    order = models.ForeignKey(
        to="Order", on_delete=models.CASCADE, verbose_name="Заказ"
    )
