...
from online_shop.settings import DEFAULT_FROM_EMAIL
...
def send_email_with_link(confirm_link: str, email: str) -> None:
    try:
        send_mail(
            ...
            message=f"Перейдите, пожалуйста, по ссылке: " f"{confirm_link}",
        ...
        _delete_inactive_user.apply_async(
            args=(email,), countdown=USER_CONFIRMATION_TIMEOUT + 5
        )
